...
In this article we use **tensor** as in `torch.Tensor` or TensorFlow - i.e. multidimensional arrays of numbers with  some additional structure. This a very specific case of would a mathematician call a tensor.
...
$\sum_i v_i M_{ij}$ (or traditionally:  $\vec{v} M$)
...
$T$ is a tensor for which $T_{ijk} = 1$ for all $i = j = k$, otherwise there are 0s.
...
- normalized probabilities (and other moments)
...
Feynman Diagrams[@kaiser_physics_2005] (for unbounded operators on infinitely-dimensional Hilbert spaces) and quantum computing.
